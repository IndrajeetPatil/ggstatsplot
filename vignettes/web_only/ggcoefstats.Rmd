...
`coxph`, `cpglm`, `cpglmm`, `crch`, `DirichReg`, `drc`, `emmGrid`, `epi.2by2`,
`ergm`, `feis`, `felm`, `fitdistr`, `flexsurvreg`, `glmc`, `glmerMod`,
`glmmTMB`, `gls`, `gam`, `Gam`, `gamlss`, `garch`, `glht`, `glm`, `glmmadmb`,
`glmmPQL`, `glmRob`, `glmrob`, `glmx`, `gmm`, `hurdle`, `ivreg`, `iv_robust`,
`lavaan`, `lm`, `lm.beta`, `lmerMod`, `lmerModLmerTest`, `lmodel2`, `lmRob`,
`lmrob`, `logitmfx`, `logitor`, `logitsf`, `LORgee`, `lrm`, `mcmc`, `mcmc.list`,
...
`rjags`, `rlm`, `rlmerMod`, `robmixglm`, `rq`, `rqss`, `slm`, `speedglm`,
...
```{r orm, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 5}
# setup
library(rms)
set.seed(123)
...
```{r logitsf, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 4}
# setup
set.seed(123)
library(logistf)
...
```{r brmsfit_multiple, warning = FALSE, message = FALSE, fig.height = 5, fig.width = 7, eval=FALSE}
# setup
set.seed(123)
library(brms)
library(mice)
...
```{r betamfx, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 5}
# setup
set.seed(123)
library(mfx)
...
```{r logitmfx, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 5}
# setup
set.seed(123)
library(mfx)
...
```{r negbinmfx, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 5}
# simulate some data
set.seed(123)
n <- 1000
...
```{r poissonmfx, warning = FALSE, message = FALSE, fig.height = 4, fig.width = 5}
# simulate some data
set.seed(123)
n <- 1000
x <- rnorm(n)
...
```{r BFBayesFactor, fig.width=10, fig.height=10, eval=FALSE}
# setup
set.seed(123)
library(BayesFactor)
library(ggstatsplot)
...
# regression-1
mod8 <- regressionBF(rating ~ ., data = attitude, progress = FALSE)
...
mod9 <- meta.ttestBF(t, N, rscale = 1, nullInterval = c(0, Inf))
...
ggstatsplot::combine_plots(
  ...
  ggcoefstats(mod6, title = "anova-1"),
  ggcoefstats(mod7, title = "anova-2"),
  ggcoefstats(mod8, title = "regression-1"),
  ggcoefstats(mod9, title = "meta-analysis"),
  ggcoefstats(mod10, title = "regression-2"),
