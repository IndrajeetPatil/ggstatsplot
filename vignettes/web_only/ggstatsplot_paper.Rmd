...
which comes in two forms:
  ...
  - **Measurement noise**: In `ggstatsplot`, the actual variation in measurements
    ...
    boxplot laid on top (Figure-1) or a histogram (Figure-4). None of the plots,
...
```{r example4, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 8, fig.cap = "**Figure-4**. Distribution of a variable shown using `gghistostats`."}
# for reproducibility
set.seed(123)
...
```
  ...
  - **Sample-to-sample statistic variation**: Although, traditionally, this
    ...
    Figure-5). This is because the interval formed by error bars correspond to
...
1. **APA guidelines** (APA, 2009) are followed (for the most part) by default:   
  ...
  - Regression results are presented with the unstandardized or standardized
    ...
    the corresponding significance level (Figure-5).
