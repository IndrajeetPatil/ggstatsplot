...
be invariant across models:
  ...
  - The output of this function will be a `ggplot2` object and, thus, it can be
    further modified (e.g., change themes, etc.) further with `ggplot2`
    functions.
...
say our hypothesis is that the IMDB ratings for a movie are predicted by a
...
believe that the relationship between these two variables is different for
...
into account the random effects structure.

## Non-linear least-squares model (`nls`)

So far we have been assuming a linear relationship between movie budget and
rating. But what is we want to also explore the possibility of a non-linear
relationship? In that case, we can run a non-linear least squares regression.
Note that you need to choose the same non-linear function (`y ~ k/x + c`)
implemented here (try out other non-linear functions, e.g. `Y ~ k * exp(-b*c)`).
...
between rating and budget, at least with the particular non-linear function we
used.

## generalized linear model (`glm`)

In all the analyses carried out thus far, the outcome variable (`y` in `y ~ x`)
has been continuous. In case the outcome variable is categorical/factor, we can use
the **generalized** form of linear model that works even if the response is a
numeric vector or a factor vector, etc. 
...
take into another important factor: the passenger class in which people were
traveling. Naively, we have reasons to believe that the effects of sex and age
might be dependent on the class. To take into account this hierarchical
structure of the data, we can run generalized linear mixed effects model with a
random slope for class.
...
might exist between survival and predictor across different passenger classes,
only the sex factor remain a significant predictor (also see the
[`ggspiestats`](https://indrajeetpatil.github.io/ggstatsplot/articles/ggspiestats.html)
vignette where we had explored this dataset).
...
regression models are suitable. To study these models, we will use
...
third-party actors who harm someone. There are four different conditions formed
out of belief (neutral, negative) and outcome (neutral, negative) for four
different vignettes, each featuring a different type of harm. The question we
are interested in is what explains variation in participants' rating:
information about intentionality, consequences, or their interaction?
We will carry out this analysis for each type of harm-
...
plotlist <- ggstatsplot::intent_morality %>%
  ...
  purrr::map(.x = .,
             .f = ~ ggstatsplot::ggcoefstats(
               ...
               ggplot2::labs(x = "probit regression coefficient",
                             y = NULL))
...
see effect of intent and outcome information on moral judgments. But a better of
accounting for such item level differences is to model them as random factors.
So, in the current section, we used ordinal mixed-effects regression model to
see how belief and outcome contribute towards variation in moral judgment
ratings-
...
ggstatsplot::ggcoefstats(
  x = ordinal::clmm(
    ...
    ggplot2::scale_y_discrete(labels = c("belief (neutral)", "interaction", "outcome (neutral)")) +
    ggplot2::labs(x = "coefficient from probit mixed-effects regression",
                  y = "fixed effects")
```
...
Let's now to run a different type of analysis: repeated measures design. To
carry out this analysis, we will first have to convert the iris dataset from
wide to long format such that there is one column corresponding to `attribute`
(which part of the calyx of a flower is being measured: `sepal` or `petal`?) and
one column corresponding to `measure` used (`length` or `width`?). Note that
this is within-subjects design since the same flower has both measures for both
attributes. The question we are interested in is the variance in measurements
explained by both of these factors.
...
`ggcoefstats`. In particular, those classes of objects are not supported for
which there is:
...
Some of the models might be supported in future.
