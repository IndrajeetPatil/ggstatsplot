...
```{r lm1, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 8}
...
## A more interesting example of a linear model (`lm`) with IMDB

For this analysis, let's use the `movies_long` dataset, which provides
information about IMDB user ratings, budget, length, MPAA categories (e.g.,
R-rated, NC-17, PG-13, etc.), and genre for `r nrow(movies_long)` movies. Let's
say our hypothesis is that the IMDB ratings for a movie are predicted by a
combination of the movie's budget and the running time or length of the movie.

Bearing in mind what we learned from our first simple example let's first inspect the results from the `lm` model `lm(rating ~ budget * length, movies_long)`.  Rather than do that with a separate function call we can accomplish it right within our call to `ggcoefstats` by changing the `output` option from the default `"plot"` to `"tidy"`. While we're at it we'll also set `conf.level = 0.99` to be more conservative since we have plenty of observations and power to detect differences.

```{r lm2, warning = FALSE, message = FALSE, fig.height = 8, fig.width = 8}
# show "tidy" option
ggstatsplot::ggcoefstats(
  x = stats::lm(
    formula = rating ~ budget * length,
    data = movies_long
  ),
  conf.level = 0.99,
  output = "tidy")
...
```{r lm3, warning = FALSE, message = FALSE, fig.height = 8, fig.width = 8}
# plot
ggstatsplot::ggcoefstats(
  x = stats::lm(
    formula = scale(rating) ~ scale(budget) * scale(length),
    data = movies_long
  ),
  ...
  stats.label.color = c("darkgreen", "black", "red"),
  title = "Movie ratings by their genre",
  subtitle = "Source: www.imdb.com"
) +                                    
  # further modification with the ggplot2 commands
  # note the order in which the labels are entered
  ggplot2::scale_y_discrete(labels = c("Movie Budget", "Interaction Term", "Movie Length")) +
