...
# to speed up the calculation, let's use only 30% of the data
movies_30 <- dplyr::sample_frac(tbl = ggstatsplot::movies_long, size = 0.3)
...
ggstatsplot::ggcoefstats(
  ...
  package = ,                               # package from which color paletter is to be taken
  palette = ,                               # color palette for labels
...
You can also use `car` package to run omnibus ANOVA:

```{r aov3, warning = FALSE, message = FALSE, fig.height = 6, fig.width = 8}
library(car)
...
# let's use 50% of the sample to speed up the analysis
ggstatsplot::ggcoefstats(
  x = stats::aov(formula = value ~ attribute * measure + Error(id / (attribute * measure)),
             data = dplyr::sample_frac(tbl = iris_long, size = 0.5)),
  effsize = "omega",
...
ggstatsplot::combine_plots(
  ggstatsplot::ggcoefstats(
    ...
    # arguments relevant for aesthetic modification
    package = "dichromat",
