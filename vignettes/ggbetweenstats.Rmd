...
The function `ggstatsplot::ggbetweenstats` is designed to facilitate **data
exploration**, and for making highly customizable **publication-ready plots**,
...
**Note: ** that the function automatically decides whether an independent
samples t-test is preferred (for 2 groups) or a Oneway ANOVA (3 or more groups)
based on the number of levels in the grouping variable. The output of the
function is a `ggplot` object which means that it can be further modified.
...
title & caption, better x and y axis labels, and tag and label the outliers in
the data. We can and will change the overall theme as well as the color palette
in use.
...
ggstatsplot::ggbetweenstats(
  data = dplyr::filter(.data = gapminder, year == 2007, continent != "Oceania"),      # dataframe
...
## Combining plots with `combine_plots` and `ggbetweenstats`

So far we have only used a classic parametric test. The `type` (of test)
argument accepts the following abbreviations: `"p"` (for *parametric*), `"np"`
(for *nonparametric*), `"r"` (for *robust*) and `"bf"` for Bayesian. Let's run
tests with the 3 variants we haven't used so far.

The type of plot to be displayed can also be modified (`"box"`, `"violin"`, or
`"boxviolin"`). Let's produce all of these variations.
...
```{r ggbetweenstats3, message = FALSE, warning = FALSE, fig.height = 12, fig.width = 6}
library(ggstatsplot)
library(gapminder)
...
Let's focus on the same for continents and for years 1957 & 2007. We'll use the
Tufte theme and the first time through we'll group by year. Showing all 4
continents for a given year.

```{r grouped1, warning = FALSE, message = FALSE, fig.height = 11, fig.width = 7}
# for reproducibility
set.seed(123)
...
With a few small changes we can also choose to reverse the grouping and x axis
and allow the viewer to more clearly see the difference by continent between the
years.

```{r grouped2, warning = FALSE, message = FALSE, fig.height = 11, fig.width = 9}
# for reproducibility
set.seed(123)
...
As seen from the plots, although the life expectancy has been improving steadily
...
A variant of this function which will be called `ggwithinstats` is currently
under development. You *can* still use this function just to prepare the
**plot** for exploratory data analysis, but the statistical details displayed in
the subtitle will be incorrect. You can remove them by adding `+
