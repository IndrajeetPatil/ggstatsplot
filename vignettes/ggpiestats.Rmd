...
the statistical relationship(s) among one or more categorical variables. We
...
will not work with other data structures like base R tables or matrices. It can operate
...
To help demonstrate how `ggpiestats` can be used with categorical (also known as nominal)
...
line up with our hypothesis or model. To start simple and then expand, let's
say that we'd like to display a piechart with the percentages of passengers who
...
**Note:** equal proportions per category are the default, e.g. 50/50, but you can
...
Let's next investigate whether the passenger's gender was independent or
