...
quickly run the same `ggstatsplot` function across multiple levels of a
...
`purrr` package. ~~Although output from only one example is shown here, the
users are encouraged to execute the other examples.(this is inaccurate?)~~
 ...
 * If you'd like some more background or an introdcution to the (purrr packages
please see)[https://jennybc.github.io/purrr-tutorial/] this tutorial.
 ...
 * A single a `chr` string such as `xlab = "Continent"` in which case it will be
reused/recycled as many times as needed.
