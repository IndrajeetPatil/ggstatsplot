...
\texttt{ggscatterstats} & \textbf{scatterplots} & for correlations between two variables \\ \addlinespace
\texttt{ggcorrmat} & \textbf{correlation matrices} & for correlations between multiple variables \\ \addlinespace
\texttt{ggcoefstats} & \textbf{dot-and-whisker plots} & for regression models \\ \addlinespace
\bottomrule
\end{longtable}
...
\begin{longtable}[]{@{}
  ...
  >{\raggedright\arraybackslash}p{(\columnwidth - 10\tabcolsep) * \real{0.12}}@{}}
\toprule
Functions & Description & Parametric & Non-parametric & Robust & Bayes Factor \\ \addlinespace
\midrule
\endhead
...
\toprule
Functions & Type & Test & Effect size & 95\% CI available? \\ \addlinespace
\midrule
\endhead
\texttt{ggbetweenstats} (2 groups) & Parametric & Student's and Welch's \emph{t}-test & Cohen's \emph{d}, Hedge's \emph{g} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggbetweenstats} (\textgreater{} 2 groups) & Parametric & Fisher's and Welch's one-way ANOVA & \(\eta^2, \eta^2_p, \omega^2, \omega^2_p\) & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggbetweenstats} (2 groups) & Non-parametric & Mann-Whitney \emph{U}-test & \emph{r} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggbetweenstats} (\textgreater{} 2 groups) & Non-parametric & Kruskal-Wallis Rank Sum Test & \(\epsilon^2\) & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggbetweenstats} (2 groups) & Robust & Yuen's test for trimmed means & \(\xi\) & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggbetweenstats} (\textgreater{} 2 groups) & Robust & Heteroscedastic one-way ANOVA for trimmed means & \(\xi\) & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggwithinstats} (2 groups) & Parametric & Student's \emph{t}-test & Cohen's \emph{d}, Hedge's \emph{g} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggwithinstats} (\textgreater{} 2 groups) & Parametric & Fisher's one-way repeated measures ANOVA & \(\eta^2_p, \omega^2\) & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggwithinstats} (2 groups) & Non-parametric & Wilcoxon signed-rank test & \emph{r} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggwithinstats} (\textgreater{} 2 groups) & Non-parametric & Friedman rank sum test & \(W_{Kendall}\) & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggwithinstats} (2 groups) & Robust & Yuen's test on trimmed means for dependent samples & \(\delta_{R}\) & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggwithinstats} (\textgreater{} 2 groups) & Robust & Heteroscedastic one-way repeated measures ANOVA for trimmed means & \textcolor{red}{$\times$} & \textcolor{red}{$\times$} \\ \addlinespace
\texttt{ggpiestats} and \texttt{ggbarstats} (unpaired) & Parametric & \(\text{Pearson's}~ \chi^2 ~\text{test}\) & Cramér's \emph{V} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggpiestats} and \texttt{ggbarstats} (paired) & Parametric & McNemar's test & Cohen's \emph{g} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggpiestats} & Parametric & One-sample proportion test & Cramér's \emph{V} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggscatterstats} and \texttt{ggcorrmat} & Parametric & Pearson's \emph{r} & \emph{r} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggscatterstats} and \texttt{ggcorrmat} & Non-parametric & \(\text{Spearman's}~ \rho\) & \(\rho\) & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggscatterstats}and \texttt{ggcorrmat} & Robust & Percentage bend correlation & \emph{r} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{gghistostats} and \texttt{ggdotplotstats} & Parametric & One-sample \emph{t}-test & Cohen's \emph{d}, Hedge's \emph{g} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{gghistostats} & Non-parametric & One-sample Wilcoxon signed rank test & \emph{r} & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{gghistostats} and \texttt{ggdotplotstats} & Robust & One-sample percentile bootstrap & robust estimator & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\texttt{ggcoefstats} & Parametric & Regression models & \(\beta\) & \textcolor{ForestGreen}{$\checkmark$} \\ \addlinespace
\bottomrule
\end{longtable}
