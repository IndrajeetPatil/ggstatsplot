...
\hypersetup{unicode=true,
            ...
            pdfauthor={Indrajeet Patil; Fiery Cushman; Mina Cikara},
            pdfborder={0 0 0},
            breaklinks=true}
...
\setlength{\droptitle}{-2em}
  ...
  \posttitle{\par}
    \author{Indrajeet Patil\footnote{Harvard University,
  \href{mailto:patilindrajeet.science@gmail.com}{\nolinkurl{patilindrajeet.science@gmail.com}}} \\ Fiery Cushman\footnote{Harvard University,
  \href{mailto:cushman@fas.harvard.edu}{\nolinkurl{cushman@fas.harvard.edu}}
} \\ Mina Cikara\footnote{Harvard University,
  \href{mailto:mcikara@fas.harvard.edu}{\nolinkurl{mcikara@fas.harvard.edu}}
}}
...
\section{\texorpdfstring{\texttt{ggstatsplot} at a
glance}{ggstatsplot at a glance}}\label{ggstatsplot-at-a-glance}}
...
\item
  ...
  vibrations, fake perspective, dark grid lines, etc.)
  ({\textbf{???}})({\textbf{???}})(Tufte, 2001, Chapter 5).
\end{itemize}
...
graphics (({\textbf{???}}), Chapter 4). The theory has four key
principles:({\textbf{???}})
...
\NormalTok{ggstatsplot}\OperatorTok{::}\KeywordTok{combine_plots}\NormalTok{(}
  \CommentTok{# plot *without* any NAs}
\NormalTok{  ggstatsplot}\OperatorTok{::}\KeywordTok{ggcorrmat}\NormalTok{(}
    \DataTypeTok{data =}\NormalTok{ msleep_no_na,}
...
\item
  ...
  ({\textbf{???}}), putatively due to relying on default computer
  outputs. All \emph{p}-values displayed in \texttt{ggstatsplot} plots
  avoid this mistake. Anything less than \texttt{p\ \textless{}\ 0.001}
  is displayed as such (e.g, Figure-1). The package deems it unimportant
  how infinitesimally small the \emph{p}-values are and, instead, puts
  emphasis on the effect size magnitudes and their 95\% CIs.
\end{enumerate}

\hypertarget{short-tutorial-on-primary-functions}{%
\section{Short tutorial on primary
functions}\label{short-tutorial-on-primary-functions}}
...
Note that this function returns a \texttt{ggplot2} object and thus any
of the graphics layers can be further modified.
...
\NormalTok{  ),}
  ...
  \DataTypeTok{caption =} \KeywordTok{substitute}\NormalTok{(}\KeywordTok{paste}\NormalTok{(}
    ...
    \StringTok{":IMDb (Internet Movie Database)"}
\NormalTok{  )),}
...
\texttt{ggbetweenstats} function has an identical twin function
\texttt{ggwithinstats} for repeated measures designs that behaves in the
same fashion with a few minor tweaks introduced to properly visualize
the repeated measures design. As can be seen from an example below, the
only difference between the plot structure is that now the group means
are connected by paths to highlight the fact that these data are paired
with each other.
...
\NormalTok{ggstatsplot}\OperatorTok{::}\KeywordTok{grouped_ggwithinstats}\NormalTok{(}
  \DataTypeTok{data =}\NormalTok{ dplyr}\OperatorTok{::}\KeywordTok{filter}\NormalTok{(data_bugs, condition }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"LDLF"}\NormalTok{, }\StringTok{"LDHF"}\NormalTok{)),}
...
test (i.e., a \(\chi^2\) goodness of fit/gof test) will be displayed as
a subtitle.
...
In case of repeated measures designs, setting \texttt{paired\ =\ TRUE}
will produce results from McNemar's \(\chi^2\) test-
...
\CommentTok{#> Note: Results from one-sample proportion tests for each level of the variable}
\CommentTok{#> 2nd survey testing for equal proportions of the variable 1st survey.}
\CommentTok{#> # A tibble: 2 x 10}
\CommentTok{#>   `2nd survey` counts  perc N     Approve Disapprove `Chi-squared`    df}
\CommentTok{#>   <fct>         <int> <dbl> <chr> <chr>   <chr>              <dbl> <dbl>}
\CommentTok{#> 1 Disapprove      720   45  (n =~ 20.83%  79.17%              245      1}
\CommentTok{#> 2 Approve         880   55. (n =~ 90.23%  9.77%               570.     1}
\CommentTok{#> # ... with 2 more variables: `p-value` <dbl>, significance <chr>}
\end{Highlighting}
\end{Shaded}
...
\midrule
\endhead
Unpaired & \(n \times p\) contingency table & Pearson's \(\chi^{2}\)
test\tabularnewline
Paired & \(n \times p\) contingency table & McNemar's \(\chi^{2}\)
test\tabularnewline
...
\midrule
\endhead
Pearson's chi-squared test & Cramér's \emph{V} &
\textcolor{ForestGreen}{Yes}\tabularnewline
McNemar's test & Cohen's \emph{g} &
\textcolor{ForestGreen}{Yes}\tabularnewline
Goodness of fit & Cramér's \emph{V} &
\textcolor{ForestGreen}{Yes}\tabularnewline
\bottomrule
\end{longtable}
...
alternatively use \texttt{ggbarstats} function which has a similar
syntax-
...
And, needless to say, there is also a \texttt{grouped\_} variant of this
function-
...
\NormalTok{  relig }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Protestant"}\NormalTok{, }\StringTok{"Catholic"}\NormalTok{, }\StringTok{"None"}\NormalTok{),}
  \OperatorTok{!}\NormalTok{partyid }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"No answer"}\NormalTok{, }\StringTok{"Don't know"}\NormalTok{, }\StringTok{"Other party"}\NormalTok{)}
\NormalTok{)}
...
Parametric\strut
\end{minipage} & \begin{minipage}[t]{0.59\columnwidth}\raggedright
Cohen's \emph{d}, Hedge's \emph{g} (central-and noncentral-\emph{t}
distribution based)\strut
\end{minipage} & \begin{minipage}[t]{0.07\columnwidth}\raggedright
\textcolor{ForestGreen}{Yes}\strut
\end{minipage}\tabularnewline
\begin{minipage}[t]{0.26\columnwidth}\raggedright
...
This function is similar to \texttt{gghistostats}, but is intended to be
used when the numeric variable also has a label.
...
\texttt{ggcorrmat} makes a correlalogram (a matrix of correlation
...
Note that if there are \texttt{NA}s present in the selected variables,
the legend will display minimum, median, and maximum number of pairs
used for correlation tests.

There is a \texttt{grouped\_} variant of this function that makes it
easy to repeat the same operation across a \textbf{single} grouping
variable:
