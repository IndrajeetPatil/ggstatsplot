...
[`ggbarstats`](https://www.indrapatil.com/ggstatsplot/reference/ggbarstats.md)`(`` `` data ``=`` ``Titanic_full``,`` `` x ``=`` ``Survived``,`` `` title ``=`` ``"Passenger survival on the Titanic"``,`` `` caption ``=`` ``"Source: Titanic survival dataset"``,`` `` legend.title ``=`` ``"Survived?"`` ``)`
...
[`ggbarstats`](https://www.indrapatil.com/ggstatsplot/reference/ggbarstats.md)`(`` `` data ``=`` ``Titanic_full``,`` `` x ``=`` ``Survived``,`` `` y ``=`` ``Sex`` ``)`
...
[`grouped_ggbarstats`](https://www.indrapatil.com/ggstatsplot/reference/grouped_ggbarstats.md)`(`` `` ``` # arguments relevant for `ggbarstats()` ``` `` data ``=`` ``Titanic_full``,`` `` x ``=`` ``Survived``,`` `` y ``=`` ``Sex``,`` `` grouping.var ``=`` ``Age``,`` `` digits.perc ``=`` ``1``,`` `` palette ``=`` ``"ggsci::category10_d3"``,`` `` ``` # arguments relevant for `combine_plots()` ``` `` annotation.args ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(`` `` title ``=`` ``"Passenger survival on the Titanic by gender and age"``,`` `` caption ``=`` ``"Asterisks denote results from proportion tests; \n***: p < 0.001, ns: non-significant"`` `` ``)``,`` `` plotgrid.args ``=`` `[`list`](https://rdrr.io/r/base/list.html)`(``nrow ``=`` ``2L``)`` ``)`
...
[`ggbarstats`](https://www.indrapatil.com/ggstatsplot/reference/ggbarstats.md)`(`` `` data ``=`` ``fishing``,`` `` x ``=`` ``Fish``,`` `` y ``=`` ``Month``,`` `` counts ``=`` ``SumOfCaught``,`` `` label ``=`` ``"both"``,`` `` palette ``=`` ``"ggsci::default_jama"``,`` `` title ``=`` ``"Type fish caught by month"``,`` `` caption ``=`` ``"Source: completely made up"``,`` `` legend.title ``=`` ``"Type fish caught: "`` ``)`
...
`# create imaginary data`` ``clinical_trial`` ``<-`` ``tibble``::`[`tribble`](https://tibble.tidyverse.org/reference/tribble.html)`(`` `` ``~``SickBefore``, ``~``SickAfter``, ``~``Counts``,`` `` ``"No"``, ``"Yes"``, ``4``,`` `` ``"Yes"``, ``"No"``, ``25``,`` `` ``"Yes"``, ``"Yes"``, ``13``,`` `` ``"No"``, ``"No"``, ``92`` ``)`` `` `[`ggbarstats`](https://www.indrapatil.com/ggstatsplot/reference/ggbarstats.md)`(`` `` data ``=`` ``clinical_trial``,`` `` x ``=`` ``SickAfter``,`` `` y ``=`` ``SickBefore``,`` `` counts ``=`` ``Counts``,`` `` paired ``=`` ``TRUE``,`` `` label ``=`` ``"both"``,`` `` title ``=`` ``"Results from imaginary clinical trial"``,`` `` palette ``=`` ``"ggsci::default_ucscgb"`` ``)`
...
[`ggbarstats`](https://www.indrapatil.com/ggstatsplot/reference/ggbarstats.md)`(``mtcars``, ``am``, ``cyl``)`
