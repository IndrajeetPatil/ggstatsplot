...
`  ``sidebarPanel``(`\
`    ``selectInput``(``"x"``, ``"xcol"``, ``"X Variable"``, choices ``=`` `[`names`](https://rdrr.io/r/base/names.html)`(``iris``)``[``5``]``)``,`\
`    ``selectInput``(``"y"``, ``"ycol"``, ``"Y Variable"``, choices ``=`` `[`names`](https://rdrr.io/r/base/names.html)`(``iris``)``[``1``:``4``]``)`\
`  ``)``,`\
