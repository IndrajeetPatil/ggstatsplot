...
```{r ggcoefstats1, message = FALSE, warning = FALSE, fig.height = 6, fig.width = 8}
...
All the classes that are supported in the `broom` package with `tidy` and
`glance` methods (<https://broom.tidyverse.org/articles/available-methods.html>)
are also supported by `ggcoefstats`. Let's see few examples:

```{r ggcoefstats3, message = FALSE, warning = FALSE, fig.height = 12, fig.width = 6}
library(dplyr)
