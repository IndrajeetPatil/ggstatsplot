...
*Note*:
  ...
  - If you are on a linux machine, you will need to have OpenGL libraries
    installed (specifically, `libx11`, `mesa` and Mesa OpenGL Utility library -
    `glu`) for the dependency package `rgl` to work.

  - Additionally, `ggstatsplot` depends on `Rrmpfr`, which requires an OS
    library that might not be installed by default on TSD mpfr-devel:
...
----------- | --- | ------------------------- | --- | -----
Parametric | > 2 | $\eta_{p}^2$, $\eta^2$, $\omega_{p}^2$, $\omega^2$ | <font color="green">Yes<font> | `effectsize::omega_squared`, `effectsize::eta_squared`
...
other package maintainers (like Daniel Lüdecke, Dominique Makowski, Mattan S.
Ben-Shachar, Patrick Mair, Salvatore Mangiafico, etc.) who have patiently and
