...
Titanic_full <-
  ...
  dplyr::mutate(across(dplyr::vars("id"), ~ as.factor(.x))) |>
  split(f = .$id) |>
  purrr::map_dfr(.f = ~ tidyr::uncount(.x, n)) |>
  dplyr::mutate(across(dplyr::vars("id"), ~ as.numeric(as.character(.x)))) |>
  dplyr::mutate(across(where(is.character), ~ droplevels(as.factor(.x)))) |>
  dplyr::select(-n, -id) |>
  tibble::rownames_to_column(var = "id") |>
  dplyr::mutate(across("id", ~ as.numeric(as.character(.x))))
