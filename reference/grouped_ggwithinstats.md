...
- ...:
  ...
  : Logical that decides whether individual data points and means,
    ...
    only when there are two groups (i.e., in case of a *t*-test). In
    case of large number of data points, it is advisable to set
  ...
  : Across repeated measures conditions, each row in the dataset must
    ...
    shown in the plot, but inferential statistics will be computed using
    only complete repeated-measures pairs.
  ...
  : A list of additional aesthetic arguments passed on to
    ...
    (IQR) from the box (Tukey-style). To customize whisker length, you
    can use the `coef` parameter, e.g., `boxplot.args = list(coef = 3)`
    for whiskers extending to 3 \* IQR, or
  ...
  : The grouping (or independent) variable from `data`. In case of a
    ...
    argument is not available or not explicitly specified, the function
    assumes that the data has already been sorted by such an id by the
    user and creates an internal identifier. So if your data is **not**
    sorted, the results *can* be inaccurate when there are more than two
    levels in `x` and there are `NA`s present. The data is expected to
    be sorted by user in subject-1, subject-2, ..., pattern.
