...
`glmmTMB`, `gls`, `gam`, `Gam`, `gamlss`, `garch`, `glht`, `glm`,
`glmm`, `glmmadmb`, `glmmPQL`, `glmRob`, `glmrob`, `glmx`, `gmm`,
`hurdle`, `ivreg`, `iv_robust`, `lavaan`, `lm`, `lm.beta`, `lmerMod`,
`lmerModLmerTest`, `lmodel2`, `lmRob`, `lmrob`, `logitmfx`, `logitor`,
`logitsf`, `LORgee`, `lrm`, `manova`, `maov`, `mcmc`, `mcmc.list`,
`MCMCglmm`, `mclogit`, `mmclogit`, `mediate`, `metafor`, `metaplus`,
