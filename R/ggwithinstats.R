...
ggbetweenstats <- function(data = NULL,
  ...
  if (test == "anova") {
    ...
    if (paired == FALSE) {
      ...
      plot <-
        plot +
        labs(subtitle = results_subtitle(robust_aov_stat = robust_y_aov))
    ...
    if (type == "parametric") {
      ...
      vartest <-
        stats::var.test(x = as.numeric(data$x),
                        y = data$y,
      ...
      ######################################### robust t-test ############################################################
      # subtitle if it's *not* a repeated measures design
      if (paired == FALSE) {
    ...
    fun_mean <- function(x) {
      return(data.frame(
        y = as.numeric(as.character(
          ggstatsplot::specify_decimal_p(x = mean(x = x))
        )),
        label = as.numeric(as.character(
          ggstatsplot::specify_decimal_p(x = mean(x = x, na.rm = TRUE))
        ))
      ))
    }
