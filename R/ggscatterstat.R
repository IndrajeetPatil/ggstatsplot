...
ggscatterstat <- function(df, x, y, xlab = NULL, ylab = NULL, marginal = NULL,
  ...
  if (test == "pearson") {
    ...
    corr_label <- base::substitute(paste("robust regression: estimate = ", estimate, ", ", italic("t"),
  ...
  plot <- ggplot2::ggplot(df, aes(x = x, y = y)) +
    ...
    theme_mprl() + # theme_mprl() is already defined above
    labs(x = xlab, y = ylab, title = title, subtitle = corr_label, caption = caption)

  # by default intercept lines will be plotted
