...
#' @param intercept decides whether mean or median or no intercept lines are to be plotted
...
ggscatterstat <- function(df, x, y, xlab = NULL, ylab = NULL, marginal = NULL,
  ...
  if (is.null(xfill)) xfill <- "orange"
  if (is.null(yfill)) yfill <- "green"
  ...
  if (test == "pearson") {
    ...
    stat_label <- base::substitute(paste("Pearson's ", italic("r"), "(", df, ")", " = ", estimate, ", ", italic("p")," = ", pvalue),
    ...
    stat_label <- base::substitute(paste("Spearman's ", italic(rho), "(", df, ")", " = ",
    ...
    MASS_res <- MASS::rlm(scale(y) ~ scale(x), maxit = 1000, na.action = na.omit, data = df)
    stat_label <- base::substitute(paste("robust regression: estimate = ", estimate, ", ", italic("t"),
                                         "(", df, ")", " = ", t, ", ", italic("p")," = ", pvalue),
                                   list(estimate = specify_decimal(summary( MASS_res)$coefficients[[2]], 3),
                                        t = specify_decimal(summary( MASS_res)$coefficients[[6]], 3),
                                        df = summary( MASS_res)$df[2],
                                        pvalue = specify_decimal_p((sfsmisc::f.robftest( MASS_res))$p.value),3))
  }

  # preparing the scatterplotplot
  plot <- ggplot2::ggplot(df, aes(x = x, y = y)) +
    geom_count(show.legend = FALSE, color = "black", size = 3, alpha = 0.5,
               position = position_jitterdodge(jitter.width = NULL, jitter.height = 0.2, dodge.width = 0.75)) +
    geom_smooth(method = "rlm", se = TRUE, size = 1.5) + # default is robust linear model
    theme_mprl() + # theme_mprl() is already defined below
    labs(x = xlab, y = ylab, title = title, subtitle = stat_label, caption = caption)
  ...
  if (intercept == "mean") {
    ...
    plot <- plot +
      geom_vline(xintercept = mean(x), linetype = "dashed", color = xfill, size = 1.2) +
      geom_hline(yintercept = mean(y), linetype = "dashed", color = yfill, size = 1.2)
  ...
  if (is.null(marginal)) marginal <- TRUE

  if (isTRUE(marginal)) {

    # if ggmarginal marginaltype has not been specified, go to this default
    if (is.null(marginaltype)) marginaltype <- "histogram"

    # creating the ggMarginal plot of a given marginaltype
    plot <- ggExtra::ggMarginal(plot, type = marginaltype, xparams = list(fill = xfill, col = "black"),
...
theme_mprl <- function() {
  ...
  ggplot2::theme_grey() +
    ggplot2::theme(axis.title.x = element_text(size = 14, face = "bold"),
                   ...
                   plot.margin = unit(c(1,1,1,1), "lines"), # requires library(grid))
                   panel.border = element_rect(colour = "black", fill = NA, size = 1),
                   plot.title = element_text(color = "black", size = 16, face = "bold", hjust = 0.5),
                   plot.subtitle = element_text(color = "black", size = 12, face = "bold", hjust = 0.5))
