...
- `ggbarstats()` (and `grouped_ggbarstats()`) now supports one-sample
  goodness-of-fit tests when `y` is not specified (the new default). This
  produces a single stacked bar chart with chi-squared GOF test results in the
  subtitle, mirroring the existing one-sample support in `ggpiestats()` (#532,
  #697).
